...
# --- Table Header ---
#printf "%-7s | %-30s | %-60s | %-40s | %-30s | %-30s\n" \
#       "PID" "Name" "Status (Name, State, VmRSS)" "CWD" "Limits (partial)" "Mounts (partial)"
#printf "%-7s | %-30s | %-60s | %-40s | %-30s | %-30s\n" \
#       "---" "----" "---------------------------" "---" "----------------" "----------------"
...
#    MOUNTS_CONTENT="N/A"
    ...
    if [ -f "$pid_dir/status" ]; then
        if [ -r "$pid_dir/status" ]; then
            ...
            STATUS_CONTENT=$(sanitize_output "$STATUS_FULL" 60 "status")
        else
    ...
    if [ -L "$pid_dir/cwd" ]; then
        if [ -e "$pid_dir/cwd" ] && [ -r "$pid_dir/cwd" ]; then
            ...
            if [ -n "$CWD_PATH" ]; then
                CWD_PATH=$(sanitize_output "$CWD_PATH" 40)
            else
    ...
    # 4. Get Limits (first few lines)
#    if [ -f "$pid_dir/limits" ]; then
#        if [ -r "$pid_dir/limits" ]; then
#            LIMITS_CONTENT=$(head -n 2 "$pid_dir/limits" | tr '\n' ' ' ) # Get first 2 lines
#            LIMITS_CONTENT=$(sanitize_output "$LIMITS_CONTENT" 30)
#        else
#            LIMITS_CONTENT="[No Perms]"
#        fi
#    fi
...
#            MOUNTS_CONTENT=$(sanitize_output "$MOUNTS_CONTENT" 30)
    ...
    if [ -n "$PROC_NAME" ]; then
#        printf "%-7s | %-30s | %-60s | %-40s | %-30s | %-30s\n" \
        echo       "$PID" "$PROC_NAME" "$STATUS_CONTENT" "$CWD_PATH" "$ROOT_PATH" "$MOUNTS_CONTENT"
    fi
